...
pub async fn get_my_shop(req: HttpRequest, pool: web::Data<SqlitePool>) -> impl Responder {
    ...
    let shop_option = match sqlx::query_as::<_, shop>("SELECT * FROM company WHERE user_id = ?")
        .bind(user_id)
...
pub async fn get_shop_by_id(pool: web::Data<SqlitePool>, path: web::Path<i32>) -> impl Responder {
    ...
    let shop_option = match sqlx::query_as::<_, shop>("SELECT * FROM company WHERE id = ?")
        .bind(shop_id)
        .fetch_optional(pool.get_ref())
        .await
    {
...
pub async fn create_or_update_shop(
    ...
    let existing = match sqlx::query_as::<_, shop>("SELECT * FROM company WHERE user_id = ?")
        .bind(user_id)
        .fetch_optional(pool.get_ref())
        .await
    {
    ...
    match existing {
        Some(shop) => {
            ...
            let updated_shop = sqlx::query_as::<_, shop>(
                "UPDATE company SET name = ?, email = ?, phone = ?, location = ?, description = ?, logo_url = ?, banner_url = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ? RETURNING *"
            )
        ...
        None => {
            ...
            let new_shop = sqlx::query_as::<_, shop>(
                "INSERT INTO company (user_id, name, email, phone, location, description, logo_url, banner_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?) RETURNING *"
            )
            .bind(user_id)
            .bind(&name)
            .bind(&email)
...
pub async fn delete_shop(req: HttpRequest, pool: web::Data<SqlitePool>) -> impl Responder {
    ...
    let result = sqlx::query("DELETE FROM company WHERE user_id = ?")
        .bind(user_id)
        .execute(pool.get_ref())
        .await;
